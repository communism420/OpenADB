...
release. It describes the automated 3.1.0 pipeline and the manual evidence
...
- Build and release only from an immutable `v<version>` tag. OpenADB 3.1.0 is
  intentionally restricted to `v3.1.0` by the release workflow.
...
2. Update ACBridge's `AndroidManifest.xml` and `BuildInfo.java` from the same
   values.
3. Update Windows version metadata, changelogs, README references, and any
   version-specific build or test expectations. If screenshots are intentionally
   retained from an earlier design, document their capture version instead of
   renaming unchanged image files.
4. Search for stale identifiers before building:

   ```powershell
   rg -n "3\.0\.3|30301|OpenADB-3\.0\.3|ACBridge-3\.0\.3" .
   ```
...
For OpenADB 3.1.0 the required helper identity is
`com.communism420.acbridge`, `versionName=3.1.0`, and `versionCode=31009`.
...
`tools/build_acbridge.py` first verifies the pinned Shizuku API 13.1.5 AARs
and core-library desugaring 2.1.5 artifacts documented under
`openadb/resources/acbridge/third_party/`. It then compiles the reviewed Java
sources, creates DEX,
...
& "$($buildTools.FullName)\aapt.exe" dump badging openadb\resources\acbridge\ACBridge-3.1.0.apk
& "$($buildTools.FullName)\zipalign.exe" -c -v 4 openadb\resources\acbridge\ACBridge-3.1.0.apk
java -jar "$($buildTools.FullName)\lib\apksigner.jar" verify --verbose --print-certs openadb\resources\acbridge\ACBridge-3.1.0.apk
```
...
authenticity. ACBridge itself is not a debuggable application. Its exported
P2P service requires Android's shell-only `DUMP` permission, verifies the
LocalSocket peer UID, and receives one-shot control data through a temporary
request-scoped ADB forward without `run-as` or status files. Its Shizuku
control activity is also DUMP-protected; it requests user approval through the
official Shizuku API and executes bounded request-scoped work in a UserService.
The public debug
signing identity is not the Windows Authenticode certificate and must never be
reused as one. Do not rotate either installed identity as an incidental build
fix, and never add a private publisher keystore, PFX, password, or certificate
data to the repository or workflow logs.
...
`OpenADB.spec` produces a one-file `OpenADB-3.1.0.exe` build intermediate and
...
`OpenADB-3.1.0-unsigned.exe`. Do not commit the large EXE; publish it as an
...
the release workflow, and the exact `v3.1.0` tag. Its smoke test uses a clean
temporary OpenADB profile and a read-only startup path. It checks:

- process startup and clean shutdown;
- the exact `OpenADB 3.1.0` window title;
- bundled ADB, fastboot, Platform Tools libraries, and notice;
- bundled ACBridge package/version metadata;
- absence of a crash log.

The workflow uploads either `OpenADB-3.1.0-windows-signed` or
`OpenADB-3.1.0-windows-unsigned`. The artifact contains exactly one
...
3. sign the temporary `OpenADB-3.1.0-unsigned.exe` candidate with SHA-256 and
   the configured timestamp;
4. run `signtool verify /pa /all /v /tw` and require exit code zero;
5. only after verification, rename it to the stable `OpenADB-3.1.0.exe`;
...
```powershell
$executables = @(Get-ChildItem . -File -Filter 'OpenADB-3.1.0*.exe')
...
```powershell
if ($executables[0].Name -ne 'OpenADB-3.1.0.exe') { throw 'Unsigned EXE cannot pass the signed gate.' }
signtool verify /pa /all /v /tw .\OpenADB-3.1.0.exe
```
...
```powershell
git status --short
git tag -a v3.1.0 -m "OpenADB 3.1.0"
git show --no-patch --decorate v3.1.0
...
Release notes are generated from the English 3.1.0 changelog section and add:
...
`OpenADB-3.1.0-unsigned.exe`, records `"signed": false`, and never uses the
...
record, select the `v3.1.0` ref in Actions, manually dispatch
`OpenADB 3.1.0 release`, and enable `allow_unsigned_stable`. That explicit
...
Never rename an unsigned EXE to `OpenADB-3.1.0.exe`, manually set
