...
Проверенная версия: OpenADB 1.1.0
...
В исходной папке отсутствовал каталог или файл `.git`. Поэтому до восстановления метаданных Git локальные ветка, индекс и история были недоступны. Удалённый `main` указывает на `7c6471a` (`Update OpenADB 1.1.0 features and UI fixes`); побайтовое сравнение отслеживаемого дерева показало, что исходный локальный снимок совпадал с этим коммитом. Последние доступные коммиты: `7c6471a`, `a9852d3`, `57bc6e6`.
...
| Штатный запуск с Platform Tools без устройства | Окно `OpenADB 1.1.0` появилось, tools Found, no-device состояние; обычное закрытие дало exit code 0, stderr пуст. |
