...
class VersionMetadataTests(unittest.TestCase):
    def test_openadb_public_version_is_consistent(self) -> None:
        ...
        self.assertEqual(RELEASE_EXE_FILENAME, "OpenADB-3.0.3.exe")
        self.assertIn("Version: `3.0.3`", (ROOT / "README.md").read_text(encoding="utf-8"))
        self.assertIn("## [3.0.3]", (ROOT / "CHANGELOG.md").read_text(encoding="utf-8"))
        self.assertIn("## [3.0.3]", (ROOT / "CHANGELOG_EN.md").read_text(encoding="utf-8"))
        self.assertIn("OpenADB 3.0.3", (ROOT / "GUI_AUDIT.md").read_text(encoding="utf-8"))
        self.assertIn("OpenADB 3.0.3", (ROOT / "GUI_REDESIGN_REPORT.md").read_text(encoding="utf-8"))

    def test_android_version_code_policy_is_documented_and_monotonic(self) -> None:
        self.assertEqual(VERSION_PARTS, (3, 0, 3))
        ...
        self.assertEqual(android_version_code(VERSION_PARTS, ACBRIDGE_BUILD), 30301)
        self.assertEqual(ACBRIDGE_VERSION_CODE, 30301)
        self.assertGreater(ACBRIDGE_VERSION_CODE, 30201)
    ...
    def test_bundled_apks_are_real_current_signed_builds(self) -> None:
        ...
        self.assertFalse((BRIDGE_ROOT / "ACBridge-3.0.2.apk").exists())
    ...
    def test_pyinstaller_and_windows_metadata_match_release_artifact(self) -> None:
        ...
        self.assertNotIn("ACBridge-3.0.2.apk", spec)

        windows_metadata = (ROOT / "tools" / "openadb_version_info.txt").read_text(encoding="utf-8")
        self.assertIn("filevers=(3, 0, 3, 0)", windows_metadata)
        self.assertIn("prodvers=(3, 0, 3, 0)", windows_metadata)
        self.assertIn("FileVersion', '3.0.3'", windows_metadata)
        self.assertIn(f"OriginalFilename', '{RELEASE_EXE_FILENAME}'", windows_metadata)
        self.assertIn("ProductVersion', '3.0.3'", windows_metadata)
        ...
        self.assertNotIn(r"^## \[3\.0\.2\]", release_workflow)
    ...
    def test_release_screenshot_names_match_version(self) -> None:
        ...
        prior_screenshots = {name.replace("v3.0.3", "v3.0.2") for name in EXPECTED_SCREENSHOTS}
        ...
        self.assertNotIn("-v3.0.2.png", readme)
